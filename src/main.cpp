...
class ZoomClient: public ZoomMSDeviceListener {

public:
    void onDeviceReady(const __FlashStringHelper * device_name, 
                       char * fw_version) {
    ...
    void onTunerEnabled(bool aEnabled) {
        updateDisplay();
    }
...
void onNextClicked() {
    if(zoom.tunerEnabled() == false && !_isScrolling) {
        zoom.incPatch(1);
    }
    _btNextDown = false;
}
